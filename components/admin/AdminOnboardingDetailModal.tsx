...
/**
 * Pendant admin de StudentOnboardingDetailModal (/profil) : même contenu,
 * mais `studentId` explicite (le coach consulte la fiche d'un autre
 * compte) et sauvegarde avec `skipEmptyFields: true` — un champ laissé
 * vide dans ce formulaire ne doit jamais écraser une réponse que l'élève a
 * déjà renseignée lui-même via /onboarding ou /profil.
 */
export function AdminOnboardingDetailModal({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const details = await getStudentOnboardingDetails(supabase, studentId);
      if (!cancelled) {
        setProfile(details);
        setLoading(false);
      }
  ...
  async function handleSave() {
    ...
    const success = await updateStudentOnboardingDetails(supabase, studentId, onboardingFormToSubmission(form), {
      ...
      weightEntrySource: "coach_update",
    });
    setSaving(false);
    if (!success) {
      setError(true);
              ...
              {!loading && editing && (
                <div className="flex flex-col gap-4">
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    Un champ laissé vide ici n&apos;écrase pas une réponse déjà renseignée par l&apos;élève.
                  </p>
