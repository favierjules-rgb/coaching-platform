-- ============================================================================
-- N1.3 — LES OCCURRENCES DE LISTES ENTRENT DANS LA SAUVEGARDE DU PLAN
-- ============================================================================
--
-- DEUX CHANGEMENTS, UNE SEULE MIGRATION. Ils ne se recouvrent pas : une
-- fonction, une policy. Aucune table, aucune colonne, aucun index, aucun
-- trigger, aucun backfill, aucune donnée réécrite.
--
-- ────────────────────────────────────────────────────────────────────────────
-- 1. `save_nutrition_plan_v2` APPREND LES OCCURRENCES
-- ────────────────────────────────────────────────────────────────────────────
-- Même nom, même signature, même garde, même retour. Une clé OPTIONNELLE
-- `choice_slots` apparaît sur chaque repas :
--
--   "meals": [{ "id": "…", "slot": "dinner", …,
--     "choice_slots": [{
--       "id": "uuid|null", "label": "Choix de ta protéine",
--       "source_list_id": "uuid|null",
--       "options": [{"catalog_food_id": "uuid"}, {"product_id": "uuid"}] }] }]
--
--   clé absente  → les occurrences de ce repas ne sont PAS touchées ;
--   []           → toutes les occurrences de ce repas sont retirées ;
--   non vide     → synchronisation complète, options comprises.
--
-- ⚠️ POURQUOI DANS CETTE FONCTION ET PAS À CÔTÉ. Une fonction plpgsql est UNE
-- transaction. Écrire le repas ici et ses occurrences depuis le navigateur
-- rendrait possibles « repas enregistré, deux snapshots sur trois » et
-- « occurrence créée sans ses options ». En les écrivant ici, ces états
-- cessent d'être des risques à surveiller : la base ne sait pas les produire.
--
-- ⚠️ CE QUI N'EST PAS FAIT. `meals.items` n'est pas touché d'une ligne, aucun
-- ancien plan n'est converti, aucune occurrence n'est créée d'office nulle
-- part. `enregistrer_repas_planifie`, `supprimer_repas_planifie` et
-- `cible_creneau_du_repas` ne sont pas modifiées.
--
-- ────────────────────────────────────────────────────────────────────────────
-- 2. UN COACH NE DÉCLARE COMME PROVENANCE QUE SES PROPRES LISTES
-- ────────────────────────────────────────────────────────────────────────────
-- `meal_choice_slots_manage_staff` ne vérifiait que le RÔLE. Un coach pouvait
-- donc écrire dans `source_list_id` l'identifiant d'une liste privée d'un
-- confrère. La portée fonctionnelle est nulle — cette colonne est de la
-- provenance, jamais lue pour afficher ni pour calculer — mais « nulle » n'est
-- pas « impossible », et la fonction ci-dessus étant `security invoker`, le
-- `with check` la couvre elle aussi.
--
          ...
          if v_meal ? 'choice_slots' then
            ...
            for v_occ in select * from jsonb_array_elements(v_occurrences) loop
              ...
              for v_option in select * from jsonb_array_elements(v_options) loop
                ...
                if not found then
                  insert into public.meal_choice_options (slot_id, position, catalog_food_id, product_id)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- LA POLICY D'ÉCRITURE DES OCCURRENCES
-- ═══════════════════════════════════════════════════════════════════════════
--
-- ⚠️ TROIS BRANCHES, ET CHACUNE EST NÉCESSAIRE.
--
--   `source_list_id is null`  — une occurrence sans provenance reste valide.
--     C'est le cas prévu par N1.1 : `on delete set null` y amène tout seul
--     quand une bibliothèque disparaît, et le snapshot doit y survivre.
--
--   `is_admin()`              — `current_coach_id()` rend NULL pour un
--     administrateur qui n'a pas de ligne dans `coaches`. Sans cette branche,
--     cette migration mettrait l'administration dehors.
--
--   propriétaire de la liste  — la règle demandée.
--
-- ⚠️ AUCUNE CONDITION SUR `archived_at`. Une liste archivée reste une
-- provenance parfaitement valide, et le `with check` ne s'évalue qu'à
-- l'écriture d'une occurrence : une occurrence déjà posée n'est jamais
-- revalidée. Archiver ou supprimer une liste après coup ne peut donc rien
-- casser dans un repas déjà construit.
--
-- `meal_choice_options_manage_staff` n'est PAS touchée : une option ne porte
-- aucune référence possédée, seulement des identités de catalogue que tout le
-- monde peut déjà lire.
...
comment on function public.save_nutrition_plan_v2(jsonb) is
  'Sauvegarde ATOMIQUE d''un plan nutrition v2 COMPLET : plan, N profils, six créneaux par profil, sept jours rattachés à un profil, repas prescrits, et — N1.3 — les occurrences de listes de chaque repas avec leurs options snapshotées, dans UNE transaction. Une clé absente de la charge utile ne touche à rien : `choice_slots` omise laisse les occurrences du repas intactes, ce qui rend valides à l''identique toutes les charges utiles écrites avant N1.3. La position d''une occurrence et celle d''une option sont DÉRIVÉES de l''ordre du tableau, jamais lues. Un identifiant d''occurrence appartenant à un autre repas est refusé (OCCURRENCE_HORS_REPAS) ; une occurrence sans option est refusée (OCCURRENCE_SANS_OPTION) ; une option sans identité de base est refusée (OPTION_SANS_IDENTITE). security invoker, search_path vide, garde is_coach_or_admin, EXECUTE réservé à authenticated. N''accepte aucune quantité calculée par le solveur, aucune macro, aucun rôle.';
