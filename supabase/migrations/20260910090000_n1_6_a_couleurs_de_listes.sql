-- ============================================================================
-- Migration 20260910090000 — N1.6A : LA COULEUR D'UNE LISTE D'ALIMENTS.
-- (chantier feat/nutrition-structured-meals)
--
-- ────────────────────────────────────────────────────────────────────────────
-- POURQUOI
-- ────────────────────────────────────────────────────────────────────────────
-- Un coach construit un repas avec cinq listes — « Ta protéine », « Ton
-- féculent », « Tes légumes »… — et rien ne les distingue au premier coup
-- d'œil, ni dans sa bibliothèque, ni dans le constructeur, ni sur l'écran de
-- l'élève. Une couleur les sépare instantanément.
--
-- ────────────────────────────────────────────────────────────────────────────
-- CE QUE LA COULEUR N'EST PAS, ET CE POINT EST LE PLUS IMPORTANT DU LOT
-- ────────────────────────────────────────────────────────────────────────────
-- ⚠️ ELLE EST PUREMENT VISUELLE. Elle ne définit AUCUN rôle nutritionnel,
-- n'entre dans AUCUN calcul, ne touche ni les macros, ni les portions
-- préférées, ni les minimums, et ne crée AUCUNE catégorie métier implicite.
--
-- Tout ce chantier tient depuis N1.5 sur un refus : celui de décider à la
-- place du coach que « poulet » sert les protéines. Une liste peinte en rouge
-- ne devient pas une liste de protéines — elle devient une liste rouge. Le
-- jour où un calcul lirait cette colonne, la garantie tomberait ; c'est
-- pourquoi le solveur ne la reçoit même pas.
--
-- ────────────────────────────────────────────────────────────────────────────
-- LE VOCABULAIRE N'EST PAS INVENTÉ — IL EXISTE DÉJÀ
-- ────────────────────────────────────────────────────────────────────────────
-- `training_blocks.color_key` porte depuis le chantier multi-blocs exactement
-- ces sept clés, avec sa table de styles et son sélecteur accessible. C'était
-- la SEULE couleur stockée du projet ; on la réutilise mot pour mot plutôt
-- que d'ouvrir un second système. Le design system, lui, est strictement
-- monochrome plus trois accents sémantiques (destructive / warning / success).
--
-- ⚠️ AUCUNE CHAÎNE CSS ARBITRAIRE. Un `text` libre autoriserait `#000` sur
-- fond noir, et il n'existe aucun endroit où le valider.
--
-- ────────────────────────────────────────────────────────────────────────────
-- CE QUE CETTE MIGRATION NE FAIT PAS
-- ────────────────────────────────────────────────────────────────────────────
--   - AUCUN BACKFILL. Les 12 listes existantes restent à `null` et s'affichent
--     exactement comme avant ce lot ;
--   - AUCUN DEFAULT. `null` veut dire « le coach n'a rien choisi », et c'est un
--     état différent de `gray` — qui, lui, est un gris CHOISI, avec sa
--     pastille. Les confondre interdirait de revenir en arrière ;
--   - AUCUNE COLONNE SUPPRIMÉE, AUCUNE RENOMMÉE ;
--   - AUCUNE POLICY créée ni modifiée. Les colonnes héritent ;
--   - AUCUNE ÉCRITURE DE CONSOMMATION.
--
-- ⚠️ NE PAS exécuter en Production sans runbook validé.
-- ============================================================================

-- ────────────────────────────────────────────────────────────────────────────
-- A. LA COULEUR DE LA LISTE, CÔTÉ BIBLIOTHÈQUE
-- ────────────────────────────────────────────────────────────────────────────
alter table public.food_lists
  add column if not exists color_key text;

alter table public.food_lists
  drop constraint if exists food_lists_color_key_check;
alter table public.food_lists
  add constraint food_lists_color_key_check
  check (color_key is null or color_key in ('gray', 'red', 'orange', 'yellow', 'green', 'blue', 'purple'));

comment on column public.food_lists.color_key is
  'N1.6A — couleur d''affichage de la liste, PUREMENT VISUELLE. Vocabulaire identique à training_blocks.color_key (gray, red, orange, yellow, green, blue, purple) : un seul système de couleurs dans le projet. NULL = aucune couleur choisie, et c''est un état DIFFÉRENT de ''gray'' (gris choisi) — l''un n''affiche aucun accent, l''autre affiche une pastille grise. Ne définit AUCUN rôle nutritionnel, n''entre dans AUCUN calcul, ne crée AUCUNE catégorie métier. Le solveur ne la reçoit jamais.';

-- ────────────────────────────────────────────────────────────────────────────
-- B. LA COULEUR FIGÉE DANS LE REPAS
-- ────────────────────────────────────────────────────────────────────────────
-- ⚠️ LE SNAPSHOT N'EST PAS UNE OPTIMISATION, IL EST OBLIGATOIRE. Vérifié sur
-- les policies : il n'existe AUCUNE policy `select` sur `food_lists` pour un
-- élève — seulement `food_lists_manage_own_coach` et `food_lists_manage_admin`.
-- Un élève ne peut donc structurellement pas lire la couleur de la
-- bibliothèque. Sans cette colonne, la couleur serait simplement invisible de
-- son côté.
--
-- ⚠️ ET C'EST LA MÊME SÉMANTIQUE QUE TOUT LE RESTE DU CHANTIER. Comme
-- `preferred_quantity` (N1.5.1) et `minimum_quantity` (N1.5.2) : figée au
-- moment où le coach ajoute la liste au repas. Repeindre la bibliothèque
-- ensuite ne touche AUCUN repas déjà construit ; une occurrence ajoutée après
-- prend la nouvelle couleur.
alter table public.meal_choice_slots
  add column if not exists color_key text;

alter table public.meal_choice_slots
  drop constraint if exists meal_choice_slots_color_key_check;
alter table public.meal_choice_slots
  add constraint meal_choice_slots_color_key_check
  check (color_key is null or color_key in ('gray', 'red', 'orange', 'yellow', 'green', 'blue', 'purple'));

comment on column public.meal_choice_slots.color_key is
  'N1.6A — couleur FIGÉE au moment où le coach a ajouté la liste au repas. Comme preferred_quantity et minimum_quantity, elle ne suit pas la bibliothèque : repeindre la liste plus tard ne touche aucun repas déjà construit. NULL = aucune couleur. C''est la SEULE source de couleur côté élève — aucune policy select n''existe sur food_lists pour un élève, donc lire la bibliothèque lui est structurellement impossible.';

-- ────────────────────────────────────────────────────────────────────────────
-- C. `save_nutrition_plan_v2` — LA COULEUR ENTRE DANS LE SNAPSHOT
-- ────────────────────────────────────────────────────────────────────────────
-- ⚠️ AJOUT STRICTEMENT ADDITIF. Une variable, une clé lue, une colonne écrite.
-- Aucune signature ne change, aucun refus n'est ajouté, aucune charge utile
-- existante ne devient invalide : `color_key` absente laisse la colonne nulle,
-- exactement comme `choice_slots` absente laisse les occurrences intactes.
--
-- ⚠️ LA RPC NE RÉSOUT TOUJOURS RIEN. Elle ne lit ni `food_lists`, ni
-- `food_list_items` : la couleur arrive DÉJÀ RÉSOLUE dans la charge utile,
-- comme la portion et le minimum. C'est ce qui rend l'instantané insensible à
-- la bibliothèque, et le contrôle `P-G` de la checklist N1.5.1 l'épingle.

-- La fonction est reproduite depuis N1.5.2 avec pour SEULES évolutions :
--   - `v_occ_color text` ;
--   - lecture de `v_occ->>'color_key'` ;
--   - `color_key` à l'insert ET à l'update d'occurrence.
-- Le reste est identique au caractère près.
