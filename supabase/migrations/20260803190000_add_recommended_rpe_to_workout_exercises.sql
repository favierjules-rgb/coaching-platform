-- ============================================================================
-- Migration 20260803190000 — Prescription RPE par exercice (« RPE CIBLE »)
-- (chantier feat/student-previous-set-performance, volet builder).
--
-- BESOIN PRODUIT : le coach prescrit un RPE cible par exercice de
-- musculation, en valeur unique (« 8 » → toutes les séries) ou en séquence
-- par série (« 8-8-9 » → série 1 : 8, série 2 : 8, série 3 : 9). Champ
-- libre nullable, même famille que reps/recommended_load (texte prescrit).
--
-- CONTENU :
--   1. colonne nullable public.workout_exercises.recommended_rpe text —
--      convention du dépôt (recommended_load) ; NULL/'' = aucune
--      prescription. AUCUN défaut, AUCUN backfill : les lignes existantes
--      restent strictement intactes ;
--   2. save_training_session_blocks : recréée (create or replace, signature
--      inchangée) pour écrire/relire recommended_rpe — UPDATE, INSERT et
--      modèle canonique retourné. Le corps est celui de la migration
--      20260722120000 (v2 session_patch), à l'identique hors ces ajouts ;
--   3. provision_program_copy : recréée (corps de la 20260802190000, à
--      l'identique) pour copier recommended_rpe — la copie des exercices
--      utilise une liste de colonnes EXPLICITE (contrairement aux
--      prescriptions en %rowtype), la nouvelle colonne doit donc y être
--      ajoutée, sinon les copies individuelles et achats la perdraient.
--
-- GARANTIES CONSERVÉES À L'IDENTIQUE :
--   - save_training_session_blocks : security invoker, search_path vide,
--     garde is_coach_or_admin, verrou FOR UPDATE + contrôle STALE,
--     validation stricte des ids et de session_patch, revoke public/anon,
--     grant authenticated ;
--   - provision_program_copy : SECURITY DEFINER, search_path vide, garde
--     staff par profiles.user_id = auth.uid(), service_role autorisé,
--     verrou advisory, échelle d'idempotence, privilèges authenticated +
...
-- ── 1. Colonne de prescription ──────────────────────────────────────────────

alter table public.workout_exercises
  add column if not exists recommended_rpe text;

comment on column public.workout_exercises.recommended_rpe is
  'RPE cible prescrit par le coach : valeur unique ("8" — toutes les séries) ou séquence par série ("8-8-9", série N = N-ième valeur, séries au-delà de la séquence sans prescription). Valeurs entières 1-10. NULL/'''' = aucune prescription (tout l''historique antérieur au chantier feat/student-previous-set-performance).';

-- ── 2. save_training_session_blocks : + recommended_rpe ─────────────────────
...
comment on function public.save_training_session_blocks(jsonb) is
  'Moteur d''écriture canonique multi-blocs v3 : v2 (Lot 3B, session_patch atomique — migration 20260722120000) + prescription recommended_rpe par exercice (migration 20260803190000). Un seul appel RPC transactionnel, contrôle STALE inchangé.';
...
-- ── 3. provision_program_copy : copier recommended_rpe ──────────────────────
    ...
    for v_week in select * from public.program_weeks where program_id = p_program_id order by week_number loop
      ...
      for v_session in select * from public.workout_sessions
        ...
        for v_block in select * from public.training_blocks
          ...
          -- recommended_rpe ajouté (migration 20260803190000) : la liste de
          -- colonnes est EXPLICITE — sans cet ajout, copies individuelles et
          -- achats perdraient la prescription RPE.
          for v_exercise in select * from public.workout_exercises we
            ...
            insert into public.workout_exercises (
              ...
              tempo, recommended_load, recommended_rpe, video_url, notes, muscle_group,
              ...
              v_exercise.tempo, v_exercise.recommended_load, v_exercise.recommended_rpe,
              v_exercise.video_url, v_exercise.notes, v_exercise.muscle_group,
