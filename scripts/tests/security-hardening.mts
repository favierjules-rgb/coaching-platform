...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 34 depuis le correctif du chemin staff de provision_program_copy
  // (migration 20260801210000, fix/program-assignment-checkbox — déclarée
  // dans le manifeste comme le veut la procédure).
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 34, "les 34 migrations doivent rester intactes");
});
...
await test("19. manifeste : empreintes exactes et borne cohérente", () => {
  ...
  assert.equal(attendues.length, 7);
