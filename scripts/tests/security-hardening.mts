...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 38 depuis la répartition nutritionnelle structurée (migration
  // 20260804090000, feat/nutrition-adaptive-recipes — déclarée dans le
  // manifeste comme le veut la procédure).
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 38, "les 38 migrations doivent rester intactes");
});
...
await test("19. manifeste : empreintes exactes et borne cohérente", () => {
  ...
  assert.equal(attendues.length, 11);
