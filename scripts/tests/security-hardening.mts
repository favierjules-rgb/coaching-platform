...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 64 depuis ALIMENTS A1 (migration 20260831090000, déclarée au manifeste
  ...
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 64, "les 64 migrations doivent rester intactes");
});
