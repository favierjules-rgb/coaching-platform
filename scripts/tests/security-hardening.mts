...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 65 depuis ALIMENTS A2 (migration 20260901090000, déclarée au manifeste
  ...
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 65, "les 65 migrations doivent rester intactes");
});
