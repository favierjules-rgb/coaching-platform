...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 63 depuis le RPE par demi-point (migration 20260830090000, déclarée au
  // manifeste comme le veut la procédure). Ce compte ne bouge QUE par ce
  // chemin : une migration qui apparaît sans passer par le manifeste doit
  // faire échouer ce test, c'est sa seule raison d'être.
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 63, "les 63 migrations doivent rester intactes");
});
