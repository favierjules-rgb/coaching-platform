...
await test("18. le baseline est HORS de supabase/migrations — impossible à pousser", () => {
  ...
  // 37 depuis la prescription RPE cible (migration 20260803190000,
  // feat/student-previous-set-performance — déclarée dans le manifeste
  // comme le veut la procédure).
  assert.equal(migrations.filter((f) => f.endsWith(".sql")).length, 37, "les 37 migrations doivent rester intactes");
});
...
await test("19. manifeste : empreintes exactes et borne cohérente", () => {
  ...
  assert.equal(attendues.length, 10);
